...
update [DEPI Final Project].[dbo].[Calender]
set [Year]=year(date)
 

update [DEPI Final Project].[dbo].[Calender]
set [Weak]=DATEPART(week, date)
...
----------------------------------------
  ...
  UPDATE [DEPI Final Project].[dbo].[Transactions]
SET [transaction_date] = 
    CASE 
        WHEN YEAR([transaction_date]) = 1996 THEN DATEFROMPARTS(2022, MONTH([transaction_date]), DAY([transaction_date]))
        WHEN YEAR([transaction_date]) = 1997 THEN DATEFROMPARTS(2023, MONTH([transaction_date]), DAY([transaction_date]))
...
SET [stock_date] = 
    CASE 
        WHEN YEAR([stock_date]) = 1996 THEN DATEFROMPARTS(2022, MONTH([transaction_date]), DAY([transaction_date]))
        WHEN YEAR([stock_date]) = 1997 THEN DATEFROMPARTS(2023, MONTH([transaction_date]), DAY([transaction_date]))
...
UPDATE a
SET [ReturnDate] = DATEADD(day, 3, b.[transaction_date])
FROM [DEPI Final Project].[dbo].[Return] a
INNER JOIN [dbo].[Transactions] b 
ON a.[ProductKey] = b.[product_id];
